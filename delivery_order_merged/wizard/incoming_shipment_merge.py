...
class delivery_order_merge(osv.osv_memory):
    ...
    _columns = {
        'shipment_ids' : fields.many2many('stock.picking',  'merge_delivery_rel', 'merge_id', 'picking_id', 'Deliveries',  domain=[('state', '!=', 'done')]),
    }
    ...
    def do_merge(self, cr, uid, ids, context=None):
        ...
        new_pick = self.pool.get('stock.picking').create(cr, uid,{
                        'state': 'draft',
                        'picking_type_id':shipments[0].picking_type_id and shipments[0].picking_type_id.id or False,
                        }
...
#                                         _('Merging is only allowed on same branch'))
            ...
            for pick in pick_ids:
            	...
            	pick_obj.action_cancel(cr, uid, [pick], context=context)
